...
/*   Updated: 2019/02/20 08:41:09 by cempassi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "unit.h"

int			NullArgWithEmptyEnv(void)
{
	t_prgm	glob;
...
int			GetOneValidName(void)
{
	...
	glob.env = NULL;
	envinit(&glob, env);
	if(ft_strequ(((t_env *)glob.env->data)->name, "Tata"))
		return (ISTRUE);
	else
		return (ISFALSE);
}

int			GetOneValidData(void)
{
	t_prgm	glob;
	char	*env[2];
	...
	glob.env = NULL;
	envinit(&glob, env);
	if(ft_strequ(((t_env *)glob.env->data)->data, "Toto"))
		return (ISTRUE);
	else
		return (ISFALSE);
}

int			GetTwoValidName(void)
{
	t_prgm	glob;
	t_env	*variable;
	char	*env[32];
	...
	env[14] = "TMPDIR=/var/folders/zz/zyxvpxvq6csfxvn_n0003ygh000zlw/T/";
	...
	while(i >= 0)
	{
		variable = (t_env *)glob.env->data;
		if(!ft_strnequ(variable->name, env[i], 4))
			return (ISFALSE);
...
int			GetTwoValidData(void)
{
	...
	{
		variable = (t_env *)glob.env->data;
		if(!ft_strnequ(variable->data, env[i] + 5, 4))
			return (ISFALSE);
		glob.env = glob.env->next;
		i--;
	}
...
int			test_envinit(void)
{
	...
	load_test(&tests, "Two Valid Name", GetTwoValidName);
	load_test(&tests, "Two Valid Data", GetTwoValidData);
